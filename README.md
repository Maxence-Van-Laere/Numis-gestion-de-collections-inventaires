...
## Fonctionnalités
- Collections: créer, lister, supprimer
- Objets: créer, lister (par collection ou tous), éditer, supprimer, déplacer d'une collection à une autre
- Informations relatives à l'objet implémentées: titre, date d'acquistion et de production, image, commentaires, appartenance à une collection
...
## Packaging (optionnel)
- Construire l’UI: `npm run build` dans `electron-app/`. Ensuite configurez Electron pour charger `dist/index.html` en prod (déjà géré par `main.js`).
- Pour packager Electron (création d’un exécutable), ajoutez un outil (ex: `electron-builder`) et un script dédié dans `package.json`.
